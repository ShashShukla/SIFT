...
def check_extrema(i, j):
    ...
    maxmask = np.greater(dog_pyramid[i][j][1: (dog_pyramid[i][j].shape[0] - 1), 1: (dog_pyramid[i][j].shape[1] - 1)],
                         threshold_max)
    minmask = np.less(dog_pyramid[i][j][1: (dog_pyramid[i][j].shape[0] - 1), 1: (dog_pyramid[i][j].shape[1] - 1)],
                      threshold_min)

    for state in iterTable:
        maxmask[:, :] = np.logical_and(maxmask, np.greater(
            dog_pyramid[i][j][1: (dog_pyramid[i][j].shape[0] - 1), 1: (dog_pyramid[i][j].shape[1] - 1)],
            dog_pyramid[i][(j + state[0])][(1 + state[1]): (dog_pyramid[i][j].shape[0] - 1 + state[1]),
            (1 + state[2]): (dog_pyramid[i][j].shape[1] - 1 + state[2])]))
        minmask[:, :] = np.logical_and(minmask, np.less(
            dog_pyramid[i][j][1: (dog_pyramid[i][j].shape[0] - 1), 1: (dog_pyramid[i][j].shape[1] - 1)],
            dog_pyramid[i][(j + state[0])][(1 + state[1]): (dog_pyramid[i][j].shape[0] - 1 + state[1]),
            (1 + state[2]): (dog_pyramid[i][j].shape[1] - 1 + state[2])]))
